...
def item_form(existing=None):
    ...
    if st.button("Save item", type="primary"):
        ...
        # Check for special characters in SKU
        import re
...
def event_form(existing=None):
    ...
    if st.checkbox("Mark as completed and add sales data"):
        ...
        # Validation for completed events
        total_payments = cash_sales + card_sales + check_sales
        if total_payments > 0 and abs(total_revenue - total_payments) > 0.01:
            st.warning(f"Payment breakdown ({total_payments:.2f}) doesn't match total revenue ({total_revenue:.2f})")
    else:
    ...
    if st.button("Save Event", type="primary"):
        ...
        if event_date < date(2020, 1, 1):
            errors.append("Event date seems too far in the past")
        if booth_fee < 0:
            errors.append("Booth fee cannot be negative")
        if status == "completed":
            if total_revenue < 0:
                errors.append("Total revenue cannot be negative")
            if any(x < 0 for x in [cash_sales, card_sales, check_sales, discounts_given, rewards_given]):
                errors.append("Financial values cannot be negative")
        ...
        try:
            ...
            st.success(f"Event saved successfully! ID: {event_id}")
            return event_id
        except Exception as e:
